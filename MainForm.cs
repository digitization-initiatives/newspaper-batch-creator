...
namespace NewspaperBatchCreator
{
    public partial class MainForm : Form
    {
        ...
        private void browseMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                if (from_folderBrowserDialog.SelectedPath != null)
                {
                    string selectedPath = from_folderBrowserDialog.SelectedPath;

                    List<string> files = new List<string>();
                    files.AddRange(Directory.GetFiles(selectedPath, "*.*", SearchOption.AllDirectories));

                    foreach (string file in files)
                    {
                        ListViewItem item = new ListViewItem(file);
                        item.SubItems.Add(STATUS.ADDED);
                        sourceFilesListView.Items.Add(item);
                    }

                    statusBar_NumberOfFilesAddedFrom.Text = $"{sourceFilesListView.Items.Count} files loaded from {selectedPath}.";

                    //Enable Assemble Batch button:
                    //createBatchButton.Enabled = true;

                    logForm.Logger(LogForm.LogType.INFO, $"{sourceFilesListView.Items.Count} files loaded from {selectedPath}.");
                }
            }
            else
            {
